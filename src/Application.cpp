...
int Application::run() {
    while (running_) {
        ...
        if (app_state_ == AppState::Playing) {
            ...
            // as the user pans. The fixed-timestep clock is drained so resuming
            // gameplay doesn't catch up on map-builder time; the wall-clock dt
            // for pan/zoom is computed here (the simulation clock isn't
            // appropriate while in the editor).
            //
            // PBD-049: the editor owns its own frame loop. We dispatch the
            // three hooks unconditionally and consume the back-request signal
            // (Esc/Backspace/B in normal mode) to drive the state machine.
            TimePoint now = Clock::now();
            float dt = std::chrono::duration<float>(now - map_builder_last_frame_).count();
            map_builder_last_frame_ = now;
            if (dt > 0.1f) dt = 0.1f; // clamp big stalls (alt-tab etc.)
            if (dt < 0.f)  dt = 0.f;

            map_builder_.process_events(running_);
            (void)clock_.advance();
        ...
        if (seconds_since(stats_start_) >= 1.0) {
            if (app_state_ != AppState::Playing) {
                ...
                if (app_state_ == AppState::MapBuilder) {
                    ...
                    const glm::vec3& cp = map_builder_.cam_pos();
...
// hands control to the MapBuilder editor (see src/editor/map_builder.{h,cpp}).
...
void Application::enter_app_state(AppState s) {
    ...
    // PBD-049: editor session teardown is owned by MapBuilder — the SDL
    // text-input close (PBD-027) and the dirty-cells flush (PBD-033) both
    // happen inside `MapBuilder::exit()`. Running before `app_state_` flips
    // so the transition itself is atomic w.r.t. the on-disk state.
    if (app_state_ == AppState::MapBuilder) {
        map_builder_.exit();
    }
...
void Application::activate_menu_selection() {
    if (app_state_ == AppState::MainMenu) {
        ...
        if (menu_selection_ == 0) {
            // Map Builder: EPIC-001 / PBD-023 scaffold, fleshed out across
            // PBD-024–044. Behaviour lives in MapBuilder (src/editor).
...
void Application::process_menu_events() {
    ...
    // already accepts both. Keep the three Ctrl+Q sites (menu, map builder,
    // gameplay) in step.
    ...
    // event pump (MapBuilder::process_events) — this function is only invoked
...
void Application::render_menu() {
    ...
    // (MapBuilder::render) that draws the streamed world under a top-down
    // camera. PBD-024 split MapBuilder out of this widget when it grew real
    // input + a world view; PBD-049 then extracted it to its own TU.
