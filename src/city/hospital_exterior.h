...
#include "city/hospital_overhaul_interiors.h"
...
// routes, public realm, public interiors, retained garage, collision-bearing
// props, runtime light anchors, and fitted texture receivers together.
inline std::vector<StartPart> bake_polished_hospital_campus() {
    ...
    const auto interiors = bake_hospital_overhaul_interiors();
    ...
    out.reserve(out.size() + logistics.size() + mobility.size() +
                public_realm.size() + interiors.size() + garage_base.size() +
                garage_polish.size());
    ...
    out.insert(out.end(), interiors.begin(), interiors.end());
