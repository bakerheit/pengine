...
// Double precision, deliberately, and therefore NOT core's kTwoPiD (which is
// float). Phase here accumulates across a whole clip at 48 kHz; a float
// two-pi puts audible drift in the tail of a long tone. Named distinctly
// because a same-named constant one namespace down is an ambiguity, and
// silently picking one precision over the other is exactly the bug.
constexpr double kTwoPiD = 6.2831853071795864769;
...
struct OnePole {
    ...
    void set(double cutoff_hz, double sr) {
        a = 1.0 - std::exp(-kTwoPiD * cutoff_hz / sr);
        if (a > 1.0) a = 1.0;
        if (a < 0.0) a = 0.0;
    }
...
PcmClip synth_tone(float hz, float seconds, float fade_seconds,
    ...
    const double phase_step = static_cast<double>(hz) * kTwoPiD / rate;
...
PcmClip synth_engine_tone(float rpm, float load, float seconds,
    ...
    for (std::size_t i = 0; i < frames; ++i) {
        table[i] = std::sin(kTwoPiD * static_cast<double>(i) /
...
PcmClip synth_tyre_scrub(float seconds, uint32_t sample_rate) {
    ...
    for (std::size_t i = 0; i < raw.size(); ++i) {
        ...
        const double wob = 1.0 + 0.28 * std::sin(kTwoPiD * 7.0 * time)
                                + 0.16 * std::sin(kTwoPiD * 11.0 * time);
...
PcmClip synth_suspension_thump(float weight, uint32_t sample_rate) {
    ...
    for (std::size_t i = 0; i < frames; ++i) {
        ...
        double s = std::sin(kTwoPiD * thump_hz * dt) * std::exp(-decay * dt)
                 + 0.35 * std::sin(kTwoPiD * thump_hz * 2.0 * dt) *
        ...
        if (w > 0.35) {
            ...
            if (rb > 0.0) {
                s += 0.42 * (w - 0.35) / 0.65 *
                     std::sin(kTwoPiD * thump_hz * 1.15 * rb) *
...
PcmClip synth_rain_bed(float seconds, uint32_t sample_rate) {
    ...
    for (std::size_t i = 0; i < raw.size(); ++i) {
        ...
        const double swell = 1.0 + 0.13 * std::sin(kTwoPiD * 0.5 * time)
                                 + 0.07 * std::sin(kTwoPiD * 1.25 * time);
...
PcmClip synth_wind_bed(float seconds, uint32_t sample_rate) {
    ...
    for (std::size_t i = 0; i < raw.size(); ++i) {
        ...
        const double gust = 0.62
                          + 0.24 * std::sin(kTwoPiD * 0.25 * time)
                          + 0.14 * std::sin(kTwoPiD * 0.75 * time + 1.1)
                          + 0.09 * std::sin(kTwoPiD * 1.5 * time + 2.3);
...
void add_note(std::vector<float>& out, double sr, double at_s, double hz,
    ...
    for (std::size_t i = n0; i < out.size(); ++i) {
        ...
        const double s = std::sin(kTwoPiD * hz * dt)
                       + 0.34 * std::sin(kTwoPiD * hz * 2.0 * dt)
                       + 0.13 * std::sin(kTwoPiD * hz * 3.0 * dt)
                       + 0.09 * std::sin(kTwoPiD * hz * 2.76 * dt);
...
PcmClip synth_lap_record_stinger(uint32_t sample_rate) {
    ...
    for (std::size_t i = 0; i < frames; ++i) {
        ...
        out[i] += static_cast<float>(
            0.16 * env * (std::sin(kTwoPiD * kBase * 4.0 * dt) +
                          0.6 * std::sin(kTwoPiD * kBase * 6.0 * dt)));
    }
