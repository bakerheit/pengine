...
struct CarModelDef {
    ...
    float pitch_inertia_scale; // multiplier on I_x (nose-up/down). >1 = harder to pitch under brake/throttle. Use to keep a truck's nose down while CoM stays high enough to roll-tip in corners.
    float roll_inertia_scale;  // multiplier on I_z (side tip). 1.0 = uniform-box roll inertia.
