...
`meshconv` takes a source file and an **output basename** (no extension), and writes one or more files next to that basename depending on what's in the source.

```sh
# Static mesh:
./build/bin/meshconv assets/models/foo.obj assets/models/foo
# Produces: assets/models/foo.emesh

# Rigged/animated model (e.g. character or weapon):
./build/bin/meshconv assets/characters/bar.fbx assets/characters/bar
# Produces: assets/characters/bar.emesh, bar.eskel, bar.eanim
```

After editing a source model, re-run `meshconv` to regenerate the cooked files — the engine will not pick up changes to source art automatically. For rigged models, remember that all three output files (`.emesh`, `.eskel`, `.eanim`) are produced and need to land in the right place.
...
assets/             Source art and cooked binary assets
  models/           Meshes (.obj source + .emesh cooked)
