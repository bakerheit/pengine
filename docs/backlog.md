...
| PBD-050 | Fix `last_loaded_cells_` snapshot ordering vs pump() | Done | Mira | Snapshot now taken pre-pump inside `pump_and_sweep()` helper; member removed. Diff is frame-local: pre vs post on either side of one pump invocation. |
