...
## The bank vault (save version 6)

Version 6 appends one row, `bank_loot_taken bank_restock_step`, from the heist at Pinatty Savings & Trust (`game/bank_heist.h`, [design](design/bank-heist.md)). `bank_loot_taken` is a bitmask over the vault's five cash piles; `bank_restock_step` is the simulation step the vault refills at, 0 when nothing is scheduled. `0 0` is a stocked vault, and it is what every v1–v5 save loads with. A robbed vault always has a restock scheduled, and never further than one cooldown (thirty minutes of play) past the save's own clock; anything else is refused as an invalid saved bank vault.

**It is saved so the vault cannot be farmed.** The wallet the take is paid into is in the save, so the vault that paid it has to be too: without the row, banking the take, saving and loading (or just relaunching) would hand back a full vault with the money already in the wallet.

**The take in hand is not saved,** and neither is the alarm. The wanted level has never been saved, so a save made mid-escape loads with no heat, no take and the vault still emptied: the same as being caught.

## Lou's page and the call (save version 7)

Version 7 adds no row. It widens the mission field on the first row to two new stages from [the pager](pager.md): `CallLou` (4), once Lou's page has arrived, and `LouCalled` (5), once Johnny has called him back from a payphone. A version 6 file holding either is refused: no version 6 build could have written one, and letting it through would make "damaged" and "from a newer build" the same thing.

**A finished delivery gets paged after a load.** `DeliveryComplete` now leads somewhere, so a save in it, including every finished v1–v6 save, waits the full ten seconds again and then pages. A player who finished the delivery before the pager existed gets the page on their next load.

**The pager itself is not saved.** A load in `CallLou` restores the objective line and marks the nearest payphone at once, but it does not put the page back on the LCD. What the pager has shown follows from the mission stage, so there is no inbox to serialise.

## What is not saved

This is a mission/player checkpoint, not a complete simulation snapshot. Extra cars parked during the session are removed on load. Traffic, pedestrians, particles, the equipped weapon, the vault door and keypad (only the heist's loot and restock are saved), aircraft/boats and unrelated world interactions are not serialized. Use the feature for mission progress and the current player/car, rather than replaying every world event.

`save_game_tests` covers file and memory roundtrips, persistent damage/fluids, overwrite, missing/corrupt/unsupported data, invalid values, preservation of an existing slot after a rejected write, and the v4 paint row: its range and consistency checks, and the stock-paint default for v1–v3; the v5 wallet row and the v6 vault row, each with its default for older saves; and the v7 mission stages, including a v6 file that claims one. `license_plate_tests` and `tractor_trailer_tests` hold the v1–v3 migrations. `ui_flow_tests` covers Continue/New Game separation and keeping title/pause open while load success is pending.
