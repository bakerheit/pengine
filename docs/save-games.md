...
`save_game_tests` covers file and memory roundtrips, persistent damage/fluids, overwrite, missing/corrupt/unsupported data, invalid values, preservation of an existing slot after a rejected write, and the v4 paint row: its range and consistency checks, and the stock-paint default for v1–v3; the v5 wallet row and the v6 vault row, each with its default for older saves. `license_plate_tests` and `tractor_trailer_tests` hold the v1–v3 migrations. `ui_flow_tests` covers Continue/New Game separation and keeping title/pause open while load success is pending.
