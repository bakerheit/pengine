...
| `pre-push` | Refuses any push to `refs/heads/main` whose `VERSION` is not ahead of the remote's. This is the backstop for everything the others cannot see: fast-forwards, cherry-picks, rebases, `--no-verify` and branch-to-main pushes. |
...
- **All of it is client-side.** `--no-verify` on both commit and push gets
  past it. A server-side check (a GitHub Action comparing `VERSION` against
  the base) would close that and does not exist yet.
