# Pinatty perimeter infill

Twelve finished background buildings occupy genuinely empty west, south, and
central Pinatty Row cells. Ten form one L around the existing construction
...
Halloway Square terrain feathers pull them away from Pinatty's 12 m datum.
...
from `kPinattyInfillParcels`; the same solid pieces feed collision, and the lot
plus building footprints feed the city map.

## Validation

`pinatty_infill_tests` checks the perimeter and central cells, terrain support, road-and-sidewalk
...
checks all 73 active lots across 2,628 pairs and verifies the 63 Pinatty lots
against terrain.
...
```sh
./build/bin/apricot --start-at -113.0 -59.2 --start-heading -174 --road-start --overhead --frames 300 --clear --save-file /tmp/apricot-pinatty-overhead-save.json --screenshot build/pinatty-infill-overhead.bmp
./build/bin/apricot --start-at -237.3 -197.0 --start-heading -174 --road-start --start-driving --frames 300 --daylight --clear --save-file /tmp/apricot-pinatty-west-save.json --screenshot build/pinatty-infill-west-street.bmp
./build/bin/apricot --start-at 156.5 93.8 --start-heading -84 --road-start --start-driving --frames 300 --daylight --clear --save-file /tmp/apricot-pinatty-south-save.json --screenshot build/pinatty-infill-south-street.bmp
./build/bin/apricot_map_lab --zoom 5 --x -80 --z -60 --layer 0 --width 1280 --height 800 --screenshot build/pinatty-infill-map.png
```
