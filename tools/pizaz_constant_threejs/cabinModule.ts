...
import {addPizazInterior,addPizazDoorInterior} from './interiorModule.js';
...
function windowPanel(parent:THREE.Object3D,name:string,outer:P2[],inner:P2[],map:Map2,side:number,m:PizazMaterials):void{
 ...
 const sealOuter=scaleLoop(inner,1.06);
 ring(parent,`${name}_painted_frame`,outer,sealOuter,map,m.paint,n);
 ring(parent,`${name}_seal`,sealOuter,inner,map,m.trim,n);
...
function addDoor(parent:THREE.Group,side:number,front:boolean,m:PizazMaterials):void{
 ...
 addPizazDoorInterior(door,skin,side,front,m);
 if(front){
...
function addScreen(parent:THREE.Group,front:boolean,m:PizazMaterials):void{
 ...
 const sealOuter=scaleLoop(inner,1.02);
 ring(parent,`${name}_body_surround`,outer,sealOuter,map,m.paint,normal);
 ring(parent,`${name}_rubber_seal`,sealOuter,inner,map,m.trim,normal);
...
export function addPizazCabin(parent:THREE.Group,m:PizazMaterials):void{
 ...
 addRoof(cabin,m);addScreen(cabin,true,m);addScreen(cabin,false,m);addPizazInterior(cabin,m);
