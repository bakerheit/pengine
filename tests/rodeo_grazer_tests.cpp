...
StaticEmesh read(const char* name) {
    StaticEmesh mesh;
    REQUIRE(read_static_emesh(asset_path(std::string("models/vehicles/rodeo_grazer/")+name+".emesh"),mesh));
...
int main() {
    constexpr auto id=PlayerCarId::RodeoGrazer;
    static_assert(static_cast<unsigned>(id)==32u); // Append-only checkpoint identity.
    REQUIRE(std::string(player_car_definition(id).brand)=="RODEO");
    ...
    REQUIRE(body.bounds.size().z>4.5f && body.bounds.size().z<5.2f);
    REQUIRE(body.bounds.max.y<1.81f && body.bounds.max.y>1.75f);
    REQUIRE(body.indices.size()/3<15000u);
    for (float side:{-1.f,1.f}) {
        const auto& door=side>0?driver:passenger;
        for (float z:{-.15f,.07f,.41f}) for (float y:{.62f,.88f}) {
        ...
        for (float z:{-.20f,.10f}) {
            REQUIRE(!covers(open,side,z,1.4f));
            REQUIRE(!covers(door,side,z,1.4f));
        }
    }
    ...
    for (const char* name:{"front_wheel","rear_wheel"}) {
        ...
        REQUIRE_NEAR(wheel.bounds.size().y,.810f,.004f);
    }
    apricot_test::pass("Grazer has two clear door apertures, six separate panes and centered wheel assets");
    ...
    REQUIRE_NEAR(tuning.half_wheelbase,1.465f,1e-6f);
    REQUIRE_NEAR(tuning.half_track,.82f,1e-6f);
    REQUIRE_NEAR(tuning.front_drive_bias,.50f,1e-6f);
    REQUIRE(tuning.suspension_travel>=.20f);
    ...
    REQUIRE(stopped);
    apricot_test::pass("Grazer accelerates, stays upright and brakes to a stop in the real vehicle simulation");
    return apricot_test::done("rodeo_grazer_tests");
}
