                    ...
                    if (glm::length(now_fwd) > 1e-4f) {
                        ...
                        if (it != last.end()) {
                            const float d = glm::degrees(std::acos(std::clamp(
                                glm::dot(unit, it->second), -1.0f, 1.0f)));
...
void a_pursuit_stays_on_a_fleeing_player() {
    ...
    // never performed. This is scoped to free-driving cruisers on purpose:
    // lane-following traffic has its own pose snaps at junctions, they predate
    // all of this, and asserting on them here would just pin someone else's bug.
